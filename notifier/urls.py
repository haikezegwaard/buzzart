...
from views import DigestView, DigestTestView, MailView
import views
admin.autodiscover()

urlpatterns = patterns('',
    url(r'(?P<pk>\d+)/mailing', MailView.as_view(template_name='mailing.html', extra_context={ 'fallback': 1}),  name='mailing'),
    url(r'(?P<pk>\d+)/test', DigestTestView.as_view(template_name='digest.html', extra_context={ 'fallback': 1}),  name='testdigest'),
